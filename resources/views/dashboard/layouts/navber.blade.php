<div class="row g-0">
    <div class="col-12 g-0  p-2 ps-4 pe-4 m-0">
        <nav class=" navbar navbar-expand-lg navbar-light my-2 p-0">
            <div class="container-fluid">
                <a href="/" class="navbar-brand"> <b>Blog</b></a>
                <ul class="navbar-nav">
                    <li class="nav-item">
                        <a class="nav-link active" href="{{url('/dashboard')}}"> Dashboard</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="#"> Blogs</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="#">Images</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="{{url('/dashboard/users')}}">Users</a>
                    </li>
                </ul>
                <div class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                        {{Auth::user()->name}}
                    </a>
                    <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
                        <li>
                            <a class="dropdown-item text-center text-dark" onclick="event.preventDefault()" id="logoutButton">
                                Logout
                            </a>
                        </li>
                    </ul>
                </div>
            </div>
        </nav>
    </div>
    <div class="col-12 text-center g-0 " style="background:white;  border-bottom:1px solid black; border-top:1px solid black;">
        <img style="max-height:420px;" class="w-100" src="assets/laravel-blog-background-image.png" alt="" />
    </div>

</div>
